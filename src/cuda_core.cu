...
#define int_as_float __int2float_rn
...
#       ifdef XMRIG_ALGO_CN_R
        if (ALGO == Algorithm::CN_R) {
            ...
            HIP_DRV_CHECK(ctx->device_id, hipLaunchKernel(
                ctx->kernel,
                grid, block2,
                args,
...
#           ifdef XMRIG_ALGO_CN_R
            if ((ctx->algorithm != algorithm) || (ctx->kernel_height != height)) {
                ...
                HIP_DRV_CHECK(ctx->device_id, hipModuleLoadDataEx(&ctx->module, ptx.data(), 0, 0, 0));
                HIP_DRV_CHECK(ctx->device_id, hipModuleGetFunction(&ctx->kernel, ctx->module, lowered_name.c_str()));
