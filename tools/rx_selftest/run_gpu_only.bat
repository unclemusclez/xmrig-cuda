...
rem Canonical test vector: 32-byte zero seed, 76-byte base input (bytes 0..75)
...
selftest.exe %SEED% %BASE% 32 6 1> gpu_out.txt 2> gpu_err.txt
echo EXIT=%ERRORLEVEL% >> gpu_err.txt
exit /b %ERRORLEVEL%
