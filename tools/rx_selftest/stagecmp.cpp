...
int main(int argc, char** argv)
{
    ...
    // --- Program 0 only: replicate the first chain step, dump the register file
    //     right after execute() (before the next chain's blake2b). This is the
    //     ground truth for the GPU's vm_states after program 0 with bfactor=0.
    {
        std::vector<uint8_t> input = base;
        embed_nonce(input, 0);
        uint8_t th[64];
        blake2b(th, sizeof(th), input.data(), input.size(), nullptr, 0);
        vm->initScratchpad(th);
        vm->run(th);  // generateProgram(th) + initialize() + execute()
        // program 0's raw program BEFORE it gets regenerated by program 2
        {
            FILE* fp0 = fopen("cpu_prog_first.bin", "wb");
            if (fp0) { fwrite(&vm->getProgram(), 1, sizeof(randomx::Program), fp0); fclose(fp0); }
            fprintf(stderr, "[stagecmp] dumped cpu_prog_first.bin (program 0 raw program)\n");
        }
        FILE* f = fopen("cpu_rf_p0.bin", "wb");
        if (f) { fwrite(vm->getRegisterFile(), 1, sizeof(randomx::RegisterFile), f); fclose(f); }
        fprintf(stderr, "[stagecmp] dumped cpu_rf_p0.bin (%zu B register file after program 0)\n", sizeof(randomx::RegisterFile));

        // The VM's own generated program (entropyBuffer + programBuffer) after run(th).
        FILE* f3 = fopen("cpu_prog_p0.bin", "wb");
        if (f3) { fwrite(&vm->getProgram(), 1, sizeof(randomx::Program), f3); fclose(f3); }
        fprintf(stderr, "[stagecmp] dumped cpu_prog_p0.bin (%zu B Program struct)\n", sizeof(randomx::Program));

        // CPU soft-AES vs hard-AES equivalence check. The VM uses soft AES
        // (InterpretedVmDefault); if the two disagree the oracle itself is wrong.
        {
            const size_t ES = 128 + ((RANDOMX_PROGRAM_SIZE_V1 * 8 + 127) / 128) * 128;
            std::vector<uint8_t> hard(ES), soft(ES);
            fillAes4Rx4<false>(th, ES, hard.data());
            fillAes4Rx4<true>(th, ES, soft.data());
            FILE* fh = fopen("cpu_fill_hard.bin", "wb");
            if (fh) { fwrite(hard.data(), 1, ES, fh); fclose(fh); }
            FILE* fs = fopen("cpu_fill_soft.bin", "wb");
            if (fs) { fwrite(soft.data(), 1, ES, fs); fclose(fs); }
            int ndiff = 0;
            for (size_t i = 0; i < ES; ++i) if (hard[i] != soft[i]) ++ndiff;
            fprintf(stderr, "[stagecmp] soft vs hard fillAes4Rx4: %d differing bytes of %zu\n", ndiff, ES);
        }
    }
